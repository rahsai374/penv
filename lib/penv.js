...
penv.writePackage = function (packages, next) {
  var desiredObj = customMixin(packages[0], packages[1]);
  ...
  fs.writeFile(penv.pkgFile, JSON.stringify(desiredObj, null, 2), next);
}


var customMixin = function(targetObj, sourceObj) {
  var resultObj = Object.assign({}, targetObj);
  ...
  for (var key in sourceObj) {
    if (!isObject(sourceObj[key])) {
      ...
      resultObj[key] = customMixin(resultObj[key], sourceObj[key]);
    }
  }
